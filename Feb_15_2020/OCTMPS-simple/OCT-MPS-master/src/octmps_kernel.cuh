...
int init_RNG(UINT64* x, UINT32* a, UINT64* xR, UINT32* aR, UINT64* xS,
     ...
     *Below
     */
    printf("JUNYAO: In the octmps_kernel.cuh-function init_RNG      by forcing to find the local directory for the safeprimes file fixed the problem\n");
    safeprimes_file = "/home/junyao/cuda-workspace/OCTMPS-simple/OCT-MPS-master/safeprimes_base32.txt";
    /*Junyao
     *Above
     *Above
     *Above
 ...
 *
 */
__global__ void CHECK_GPU(GPUThreadStates* tstates) {

	printf("CHECK_GPU________active is %d\n\n",tstates->is_active[8000]);

}

__device__ void SaveSampleWeight(FLOAT* d_sample_weight, PhotonStructGPU* photon) {
	UINT32 tid = blockIdx.x * blockDim.x + threadIdx.x;
	d_sample_weight[tid] = photon->w;

}

__device__ void SaveSampleLocation(FLOAT* d_sample_weight, PhotonStructGPU* photon) {
	UINT32 tid = blockIdx.x * blockDim.x + threadIdx.x;
	d_sample_weight[tid] = photon->z;

}



__device__ void SaveSampleActive(FLOAT* d_sample_active, UINT32	is_active) {
	UINT32 tid = blockIdx.x * blockDim.x + threadIdx.x;
	d_sample_active[tid] = is_active;
...
__global__ void CHECK_SaveSampleWeight(FLOAT* d_sample_weight) {

	for (int i=0;i<128;i++){

		printf("i=%d weight is %lf \n",i,d_sample_weight[i]);
	}
...
/*
__global__ void CHECK_Swap(int* d_new_sample,SimState* d_state) {

	printf("%d	and	%d,%d,%d\n",d_new_sample[0],d_state->x[0],d_state->xR[0],d_state->xS[0]);
...
__global__ void CHECK_Swap(int* d_new_sample,GPUThreadStates* tstates) {

	printf("%d	and	%f\n",d_new_sample[0],tstates->photon_w[0]);
...
__global__ void MakeChange(int* d_new_sample) {

	d_new_sample[0]	=	77;


	printf("the d_sample_weight is %d\n",d_new_sample[0]);
...
__global__ void CHECK_Active(UINT32* d_resampling_stop_flag,GPUThreadStates* tstates) {

	UINT32 tid = blockIdx.x * blockDim.x + threadIdx.x;

	//printf("is_active is %d\n",tstates->is_active[tid]);

	//printf("HELLO");



	if(tstates->is_active[tid]	==	1	){

		*d_resampling_stop_flag = 1;
	}
...
__global__ void OCTMPSKernel_Secondary(SimState* d_state, GPUThreadStates* tstates) {
	...
	//tstates->FstBackReflectionFlag[tid] = 0;				//disable this make the result better, why?
...
__global__ void Check_var_after_resampling(SimState* d_state, GPUThreadStates* tstates, FLOAT* d_sample_weight) {
	// photon structure stored in registers
	PhotonStructGPU photon;							//primary photon 	(JUNYAO)
	PhotonStructGPU photon_cont;					//secondary photon	(JUNYAO)
	...
	//save photon.w for resampling			JUNYAO
	SaveSampleWeight(d_sample_weight,&photon);
...
__global__ void Check_active(SimState* d_state, GPUThreadStates* tstates, FLOAT* d_sample_weight) {
	// photon structure stored in registers
	PhotonStructGPU photon;							//primary photon 	(JUNYAO)
	PhotonStructGPU photon_cont;					//secondary photon	(JUNYAO)
	...
	d_sample_weight[tid] = double(is_active);
...
__global__ void Check_location(SimState* d_state, GPUThreadStates* tstates, FLOAT* d_sample_weight) {
	// photon structure stored in registers
	PhotonStructGPU photon;							//primary photon 	(JUNYAO)
	PhotonStructGPU photon_cont;					//secondary photon	(JUNYAO)
	...
	//save photon.w for resampling			JUNYAO
	SaveSampleLocation(d_sample_weight,&photon);
...
__global__ void OCTMPSKernel(SimState* d_state, GPUThreadStates* tstates, FLOAT* d_sample_weight) {
	// photon structure stored in registers
	PhotonStructGPU photon;							//primary photon 	(JUNYAO)
	PhotonStructGPU photon_cont;					//secondary photon	(JUNYAO)
	...
	//printf("location of prob is %f, %f, %f\n",photon.x, photon.y, photon.z);


	//for (int iIndex = 0; iIndex < NUM_STEPS; ++iIndex) {			//for resampling the NUM_STEPS is 1
