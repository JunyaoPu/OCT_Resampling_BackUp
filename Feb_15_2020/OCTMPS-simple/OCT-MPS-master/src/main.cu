#include <curand_kernel.h>				//JUNYAO for cuRAND
#include "resampling.cuh"				//JUNYAO
...
static CUT_THREADPROC RunGPUi(HostThreadState *hstate) {
	printf("\n the RUNGPUi function is called (JUNYAO)\n");
    ...
    UINT32	total_num_photon_packet = n_threads;					//JUNYAO FOR ONE GPU ONLY
    ...
    ******************************************************/
    ...
    FLOAT h_sample_weight[total_num_photon_packet];
    FLOAT *d_sample_weight;
    cudaMalloc((void**)&d_sample_weight,(total_num_photon_packet)*sizeof(FLOAT));
...
*/
    ...
    int NUM_BLOCK_PER_GRID	=	int(*HostMem->n_photons_left/NUM_THREADS_PER_BLOCK);

    dim3 dimGrid(NUM_BLOCK_PER_GRID);
    ...
    //memory configuration 	----- shared memory and L1 cache      (JUNYAO)
	...
	int sample[total_num_photon_packet];

	int new_sample[total_num_photon_packet];

	//initialize samples (0 to total_num_photon_packet)
	for(int k = 0; k < total_num_photon_packet; k++){
		sample[k] = k;
	}
	...
	//allocate GPU memory
	cudaMalloc((void**)&d_sample,(total_num_photon_packet)*size_int);

	cudaMalloc((void**)&d_new_sample,(total_num_photon_packet)*size_int);
	//copy to GPU
	cudaMemcpy(d_sample,sample,(total_num_photon_packet)*size_int,cudaMemcpyHostToDevice);

	cudaMemcpy(d_new_sample,new_sample,(total_num_photon_packet)*size_int,cudaMemcpyHostToDevice);


	//initialize variables for random number on GPU
    static curandState *states = NULL;

    //allocate memory for random number
    cudaMalloc((void **)&states, sizeof(curandState) *
    		NUM_THREADS_PER_BLOCK * NUM_BLOCK_PER_GRID);




    UINT32 total_photon_test	=	total_num_photon_packet;
    UINT32*	total_photon;
    total_photon	=	&total_photon_test;



    UINT32	check_stop_flag		=	0;
    UINT32 resampling_stop_flag	=	0;
    UINT32*	d_resampling_stop_flag;
    cudaMalloc((void**)&d_resampling_stop_flag,sizeof(UINT32));
    cudaMemcpy(d_resampling_stop_flag,&resampling_stop_flag,sizeof(UINT32),cudaMemcpyHostToDevice);
    ...
    FLOAT valueVar	=	0.0;
    int resampling_num = 0;

    OCTMPSKernel << < dimGrid, dimBlock >> > (DeviceMem, tstates,d_sample_weight);
    checkCudaErrors(cudaThreadSynchronize()); // Wait for all threads to finish
    ...
    //for (int i = 1; i < 601; ++i) {
    	...
    	//check the variance of weight
        cudaMemcpy(h_sample_weight,d_sample_weight,(total_num_photon_packet)*sizeof(FLOAT),cudaMemcpyDeviceToHost);

        valueVar = var(h_sample_weight, total_num_photon_packet);

        printf("The variance of weight is %lf\n",valueVar);


        //check the variance-----do resampling or not
        if(valueVar	>= 0.000001){						//00001				best//000001

        	//printf("The variance of weight is %lf\n",valueVar);


        	/*
        	 * Before resampling, lets finish the secondary photon packets (Lets disable backscattering)
        	 */


        	OCTMPSKernel_Secondary << < dimGrid, dimBlock >> > (DeviceMem, tstates);
        	checkCudaErrors(cudaThreadSynchronize());
            ...
            //CPU random seed generation
            srand(time(NULL)+random());
            int seed = rand();

            random_setup<<<dimGrid, dimBlock >>>(states,seed);
            checkCudaErrors(cudaThreadSynchronize());



        	//GPU_rejection_v3<<<dimGrid, dimBlock >>>(d_sample, d_new_sample, d_sample_weight,total_num_photon_packet,states);
        	//checkCudaErrors(cudaThreadSynchronize());


            GPU_metropolis_v3<<<dimGrid, dimBlock >>>(d_sample, d_new_sample, d_sample_weight,total_num_photon_packet, 16 ,states);
        	checkCudaErrors(cudaThreadSynchronize());



        	//recovery the number of photon packet in the simulator after resampling
            checkCudaErrors(cudaMemcpy(d_n_photons_left, total_photon, sizeof(UINT32), cudaMemcpyHostToDevice));


        	Resampling_Swap_v2<<<dimGrid, dimBlock >>>(DeviceMem, tstates,d_new_sample);
        	checkCudaErrors(cudaThreadSynchronize());



        	printf("Resampling is implemented %d times!!!\n",resampling_num);
        	resampling_num++;
...
/*
        	Check_active<< < dimGrid, dimBlock >> > (DeviceMem, tstates, d_sample_weight);
        	//check the variance of weight
            cudaMemcpy(h_sample_weight,d_sample_weight,(total_num_photon_packet)*sizeof(FLOAT),cudaMemcpyDeviceToHost);
...
/*
        if(i>180){
        //if(i==1||i==50||i==160){
        	Check_location<< < dimGrid, dimBlock >> > (DeviceMem, tstates, d_sample_weight);
        	//check the variance of weight
            cudaMemcpy(h_sample_weight,d_sample_weight,(total_num_photon_packet)*sizeof(FLOAT),cudaMemcpyDeviceToHost);

            for(int ii = 0 ; ii<50;ii++){
            	printf("z location is %f \n",h_sample_weight[ii]);
            }
...
*/
        ...
        CHECK_Active<< < dimGrid, dimBlock >> >(d_resampling_stop_flag,tstates);
        checkCudaErrors(cudaThreadSynchronize()); // Wait for all threads to finish
        ...
        printf("[GPU %u] batch %5d, number of photons left %10d\n",
               hstate->dev_id, i, *HostMem->n_photons_left);


        checkCudaErrors(cudaMemcpy(&check_stop_flag, d_resampling_stop_flag,
                                   sizeof(UINT32), cudaMemcpyDeviceToHost));


        //check the flag and stop the program by set the n_photons_left to zero
        if(check_stop_flag	==	0){
        	*HostMem->n_photons_left = 0;
        }
    ...
    //Transfer data from Device to Host memory after simulation(JUNYAO)
    //Transfer data from Device to Host memory after simulation(JUNYAO)
...
static void DoOneSimulation(int sim_id,
    ...
    for (UINT32 i = 0; i < num_GPUs; ++i) {					//GPU=1 (JUNYAO)
        if (cc[i] != 110) {
            pthread_mutex_lock(&running_mutex);
            running_threads++;
            pthread_mutex_unlock(&running_mutex);
...
int main(int argc, char *argv[]) {
    ...
    *********************************/
    ...
    //here is the loop for B_scan(JUNYAO)
    //here is the loop for B_scan(JUNYAO)
    //here is the loop for B_scan(JUNYAO)
    //here is the loop for B_scan(JUNYAO)
    //here is the loop for B_scan(JUNYAO)
    ...
    for (simulations->probe.current_scanning_x = simulations->probe.start_x;
        ...
        for (int i = 0; i < num_GPUs; ++i) {
            ...
            if (cc[i] >= __CUDA_ARCH__) {
                ...
                //hstates[i]->n_tblks = &simulation->number_of_photons/128;									// have to change this to change the parameter of the simulator (JUNYAO)
                ...
                printf("props.multiProcessorCount is %d\n\n",props.multiProcessorCount);
                hstates[i]->n_tblks = simulations->number_of_photons/NUM_THREADS_PER_BLOCK;
    ...
    return 1;
         ...
         * * JUNYAO:	THE GPU kernel
         */
        printf("Junyao: THE GPU kernel here \n");
        ...
        //perform all the simulations			//for the sphere input which n_simulations = 100
        for (int i = 0; i < n_simulations; i++) {					//n_simlulations = 1 (JUNYAO)
